...
//                bot.sendArrayDataToAll(signalArr);
            ...
            } else if (Integer.parseInt(temp[i + 2]) > resulTime && mapFixedStavka.containsKey(temp[i + 5])) { // если время уже более того например 15 мин когда пора проверить результат и в листе результатов есть такая команда1 у которой стоит проверить результат то
...
//                    bot.sendArrayDataToAll(resultArr);
    ...
    public static Double stavka(Double kef, Double total, int total12, double naOdnuIgru) {
        if (total12 < total) {
            return naOdnuIgru * kef; // минус потомучто в расчете выше  -=  все что поле воспринимаеться как в скобках
